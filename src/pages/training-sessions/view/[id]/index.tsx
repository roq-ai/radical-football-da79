...
import { deletePlayerTrainingById } from 'apiSdk/player-trainings';
  ...
  const { data, error, isLoading, mutate } = useSWR<TrainingSessionInterface>(
    ...
    () =>
      getTrainingSessionById(id, {
        relations: ['academy', 'user', 'player_training'],
      }),
  );

  const player_trainingHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
              ...
              {hasAccess('player_training', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/player-trainings/create?training_session_id=${data?.id}`}>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.player_training?.map((record) => (
                          <Tr
                            cursor="pointer"
                            onClick={() => router.push(`/player-trainings/view/${record.id}`)}
                            key={record.id}
                          >
                            ...
                            <Td>
                              {hasAccess('player_training', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                                <NextLink
                                  onClick={(e) => e.stopPropagation()}
                                  passHref
                                  href={`/player-trainings/edit/${record.id}`}
                                >
                              ...
                              {hasAccess('player_training', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                                <IconButton
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    player_trainingHandleDelete(record.id);
                                  }}
                                  colorScheme="red"
                                  variant="outline"
                                  aria-label="edit"
